removeYearsUI <- function(id, i18n) {
  ...
  countdownDashboard(
    ...
    dashboardTitle = i18n$t('btn_adjust_remove_years'),
    i18n = i18n,
          ...
          box(
            ...
            fluidRow(
              column(8, offset = 2, selectizeInput(ns('year_to_remove'),
                                                   label = i18n$t("title_adjust_select_years"),
                                                   choices = NULL,
                                                   multiple = TRUE,
            ...
            fluidRow(
              column(
                ...
                actionButton(ns('remove_year'),
                             label = i18n$t("btn_adjust_confirm_remove"),
                             icon = icon('wrench'),
    ...
    module = function(input, output, session) {
      ...
      observeEvent(excluded_years(), {
        ...
        if (length(excluded_years()) > 0) {
          ...
          messageBox$update_message('msg_adjust_removed_year',  'success', list(years = list_years))
        } else {
          messageBox$update_message('msg_adjust_no_years_removed', 'info')
        }
      })
