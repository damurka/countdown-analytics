...
#' Run the Pooled app
#'
#' Build one file from several countries' saved datasets, then explore it and export tables. DataSuite starts it with
#' its settings in environment variables (`CDSUITE_SHINY_NAME`, `_VERSION`, `_LOCALE`, and `_SELECTED_FILE`: a folder
#' of saved datasets); they can be given as arguments instead.
#'
#' @param pre_loaded_dir A folder of saved datasets (`.rds`) to start from, or `NA`.
...
#' @export
run_app <- function(pre_loaded_dir = Sys.getenv("CDSUITE_SHINY_SELECTED_FILE", unset = NA),
                    language = Sys.getenv("CDSUITE_SHINY_LOCALE", unset = "en"),
                    app_name = Sys.getenv("CDSUITE_SHINY_NAME", unset = "Pooled"),
                    app_version = Sys.getenv("CDSUITE_SHINY_VERSION", unset = as.character(utils::packageVersion("cd2030.pooled")))) {
  options(shiny.maxRequestSize = 2 * 1024 * 1024^2)
  # the app's stylesheet, served at cd-pooled/
  shiny::addResourcePath("cd-pooled", system.file("www", package = "cd2030.pooled"))
  ...
  i18n <- shiny.i18n::init_i18n(translation_json_path = cd_translations(system.file("translation", "translation.json", package = "cd2030.pooled")))
  ...
  ui <- cd_app_ui(
    ...
    body = cd_app_body(
      ...
      tags$head(
        ...
        tags$link(rel = "stylesheet", type = "text/css", href = paste0("cd-pooled/pooled.css?v=", as.integer(file.mtime(system.file("www", "pooled.css", package = "cd2030.pooled"))))),
