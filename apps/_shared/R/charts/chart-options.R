...
cd_chart_layout <- function(p, view = NULL) {
...
cd_chart_label_defaults <- function(p, flipped = FALSE) {
...
# report (lists, or NULL when nothing is changed). `layout` is cd_chart_layout(p, view).
cd_apply_chart_options <- function(p, labels = NULL, view = NULL, layout = cd_chart_layout(p, view)) {
