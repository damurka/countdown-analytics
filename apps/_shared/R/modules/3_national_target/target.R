target_ui <- function(id, i18n) {
  ns <- NS(id)
  tagList(
    cd_tabbed_charts_ui(ns("panel"), i18n, "title_nav_global_coverage", cd_coverage_plot_ui, 
                indicators = cd_cfg("target_indicators"), showCustom = FALSE),
    uiOutput(ns("dynamic_table_downloads_ui"))
  )
}

target_server <- function(id, cache, i18n, admin_level, region = reactive(NULL), active = reactive(TRUE)) {
  ...
  moduleServer(
    ...
    module = function(input, output, session) {
      ...
      selected_indicator <- cd_table_server(
        ...
        excel_write_fun = function(wb, d) { 
          ...
          cd_add_sheet(wb, sheet_name_1, d, title = sheet_name_1)
        }
      )

      district_coverage_rate <- reactive({
        req(cache(), active(), cache()$check_inequality_params, selected_indicator())
      ...
      cd_tabbed_charts_server(
        ...
        indicators = cd_cfg("target_indicators"),
        showCustom = FALSE
      )
    }
  )
